...
public abstract class Drawable {
    ...
    public void updateToDraw(GlobalCoordinateSystem gcs) {
        toDraw = gcs.inScreen(coordX, coordY, boundingX, boundingY);
    }
    ...
    public void drawImage(Graphics2D g2d, GlobalCoordinateSystem gcs) {
        if (isSelected) {
            drawSelected(g2d, gcs);
        } else {
            drawNotSelected(g2d, gcs);
        }
    }

    protected abstract void drawSelected(Graphics2D g, GlobalCoordinateSystem gcs);

    protected abstract void drawNotSelected(Graphics2D g, GlobalCoordinateSystem gcs);



    // Mouse Interaction variables
    //      Returns true if a click with virtual coords (offsets from origin) x and y should count as a click
